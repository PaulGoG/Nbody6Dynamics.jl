...
function plot_merger_ic(result::MergerICResult, vis::VisualizationConfig)
    ...
    scatter!(ax_v, x_pc, y_pc;
        color = (:grey70, 0.3), markersize = 0.8, strokewidth = 0, rasterize = true)
    ...
    for (ci, rng) in enumerate(result.cluster_ranges)
        ...
        arrows!(ax_v, x_pc[sub], y_pc[sub],
            ...
            color = col, linewidth = 1.0, arrowsize = 4)
    ...
    for j in eachindex(dn_plot)
        push!(step_x, m_edges[j]);   push!(step_top, dn_plot[j])
