...
- The initial neighbour radii are set from the coordinate origin with the hardcoded `RS0 = 0.5` (`fpoly0.F`), several member half-mass radii wide, so every neighbour list overflows at start-up and is shrunk by the engine's repair loop. Both runs show `RSMIN` falling from 0.50 to 0.03 within the first adjustment interval. The regularisation thresholds `RMIN = 8×10⁻⁵` and `DTMIN = 2.5×10⁻⁶` are likewise fixed in units of the combined system, and `KZ(16) = 0` keeps the engine from re-deriving them.
...
**What to trust instead.** Cluster-level results before coalescence must come from the `conf.3` snapshots by original membership. `merger_ic.toml` records the particle index range of every cluster (`cluster_ranges`), and `plot_cluster_separation`, `plot_cluster_virial`, and `per_cluster_virial` already use it. Per-cluster centres, half-mass radii, and velocity dispersions computed the same way agree with the generated clusters at t = 0 and follow their tidal growth through pericentre, where the engine's global numbers do not. Two caveats apply to the membership-based virial ratio: it diverges by construction once a cluster has dissolved into the remnant, and it is dominated by kicked stellar remnants after the first supernovae unless unbound members are excluded.

**Regime.** The engine has no native multi-cluster mode: `KZ(5) = 2` (two orbiting Plummer models) is dead code in this version, since it reads list-directed input that the NAMELIST format no longer supplies. Two clusters on a Kepler orbit and three to five clusters on explicit orbits are the configurations the package handles well. A large grid such as the 27-cluster example is a cold-collapse experiment of a cluster of clusters: its combined virial ratio at t = 0 is 0.13, every member is disrupted within about two crossing times of the configuration (about ten internal crossing times), and none of the engine's diagnostics is meaningful before the remnant forms. Check the combined virial ratio of a new configuration from the first ADJUST line of `out1000`; values well below 0.5 mean global infall dominates the evolution.

**Integration accuracy.** The generated input sets `QE = 1.0`, which disables the engine's energy-error safeguard; the single-cluster inputs shipped with the package use `QE = 2×10⁻⁴`. The small runs conserved energy to better than 10⁻⁴ per adjustment interval, but a merger run gives no warning when it does not.

**Stellar masses.** `mass_total` together with `N` fixes the mean stellar mass. The shipped small configurations use N = 1000 and 10⁴ M☉, a mean of 10 M☉ that the rescaled Kroupa sampler reaches by scaling every mass up by a factor of about 17, so they contain stars of several hundred solar masses. Their stellar evolution is unphysical by construction: supernova kicks begin after 3 Myr, mass loss accounts for a quarter of the CPU time, and the membership-based virial ratios blow up when the kicked remnants leave. These files exercise the pipeline; science configurations need `N` and `mass_total` consistent with the IMF mean (about 0.6 M☉ for Kroupa over 0.08–100 M☉) or the natural `imf = "kroupa"` mode without a target mass.
