...
- **`merger.inp`** — matching NAMELIST input file: `N` = post-truncation total, `KZ(22) = 2` (N-body-unit `dat.10` input, the only supported mode), `KZ(14) = 0` (isolated), `NRAND` = effective seed, `TCRIT`/`DTADJ`/`DELTAT` from `[merger.output]`, the integration parameters `QE`, `ETAI`, `ETAR`, `NNBOPT`, `RS0`, `RMIN`, `DTMIN`, `KZ(16)` from `[merger.nbody6]` (derived from the smallest member cluster unless set; see [Input File Reference](@ref)), and:
