...
**Integration accuracy.** The generated input uses `QE = 2×10⁻⁴` by default (`[merger.nbody6]`), the same energy-error tolerance as the single-cluster inputs; earlier versions wrote `QE = 1.0`, which disabled the engine's safeguard. The small runs without binaries conserved energy to better than 10⁻⁴ per adjustment interval; with 20 % primordial binaries the same demo reached 4×10⁻³ over its first time unit, so binary-rich configurations need `qe` of order 10⁻³ to 10⁻².
