...
- **Julia 1.13+** through [juliaup](https://github.com/JuliaLang/juliaup), the version the package is developed and validated on. Manifests are not under version control: each environment resolves from its `Project.toml` on first activation, and every run stores the manifest it resolved as `environment_manifest.toml`
...
`bench/gpu_scaling.jl` runs the same two-cluster case with the CPU and the GPU binary over a grid of N, thread counts and `GPU_LIST` values, and prints the GPU speed-up at equal N and threads together with the backend's regular-force share and the kernel throughput. The regular force grows as N² against N⟨N_nb⟩ for the neighbour force, so its share, and with it the gain from the GPU, rises with N; at N = 2×10⁴ it is two thirds of the backend CPU time on the reference workstation, which bounds the gain there at about 3×. Two devices in one process (`gpu_list = [0, 1]`) halve only the regular-force term, so they pay off later in N than the first device. Under MPI every rank on a host applies the same `gpu_list`; per-rank device slicing is not provided. `bench/gpu_cells.jl` is the device-side counterpart for large cells: a grid of (cell, N, host threads, `GPU_LIST`, MPI ranks) points, the single cell being one King cluster at rest and the dual cell the two-cluster case, run with post-processing off and read back into one CSV row per run (wall time, the backend timing table, kernel throughput, GPU utilisation, power and memory, host RSS and CPU efficiency, the machine identity and the run ID); a failed run is recorded and the grid continues. `julia bench/gpu_cells.jl --cells=single,dual --n=1000000 --threads=8 --gpus="0;0,1" --tcrit=2.0` is a strong-scaling pair on a two-device node; `--cpu` adds the AVX rows and `--mpi-ranks=2` runs the device rows on an MPI+CUDA tree. `bench/gpu_cells_figures.jl` draws two figures from any set of those CSVs: the multi-device scaling of the machine that ran with two devices (strong scaling as speed-up against the device count for the regular-force share and the wall time, weak scaling with N ∝ D, host-thread scaling) and the cross-hardware chart of every card (wall time on a log axis with a tick at the regular-force share, and the kernel rate as a fraction of the FP32 peak from a `--cards` table, which also sets the axis order and the datacenter / consumer divider). The backend timers are summed over threads, so the figures use their shares of the wall time. `--synthetic` fabricates a data set to rehearse the layout.
